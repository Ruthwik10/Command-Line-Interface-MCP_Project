...
class CliChat(Chat):
    ...
    async def _process_command(self, query: str) -> bool:
        ...
        if command not in prompts:
            available = ", ".join(f"/{name}" for name in prompts) or "none"
            raise ValueError(
                f"Unknown command '/{command}'. Available commands: {available}"
            )

        arg_names = [a.name for a in (prompts[command].arguments or [])]
        supplied = words[1:]

        required = [
            a.name for a in (prompts[command].arguments or []) if a.required
        ]
        if len(supplied) < len(required):
            usage = " ".join(f"<{name}>" for name in arg_names)
            raise ValueError(f"Usage: /{command} {usage}".rstrip())

        args = dict(zip(arg_names, supplied))
